...
class Vindex:
    ...
    def __init__(
        ...
        root: Path,
        cfg: VindexConfig,
        attn_weights: Optional[Union[str, Path]] = None,
    ) -> None:
        self.root = root
        self.config = cfg
        h = cfg.hidden_size
        ...
        attn_path = Path(attn_weights) if attn_weights is not None else root / "attn_weights.bin"
    ...
    @classmethod
    def load(cls, path: str | Path, attn_weights: Optional[Union[str, Path]] = None) -> "Vindex":
        root = Path(path)
        log.info("Chargement Vindex depuis %s", root.resolve())
        if attn_weights is not None:
            log.info("Poids attention: %s", Path(attn_weights).resolve())
        raw = json.loads((root / "index.json").read_text(encoding="utf-8"))
        v = cls(root, VindexConfig.from_json(raw), attn_weights=attn_weights)
        log.info(
...
def infer(
    ...
    *,
    ...
    if not skip_attention and vindex.attn is None:
        log.warning(
            "attn_weights.bin absent (ou chemin --attn-weights invalide) — attention ignorée, "
            "FFN exécuté (même sémantique que vindex-infer Rust). "
            "Pour attention+FFN, fournissez le fichier ou --attn-weights."
        )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Chemin vers attn_weights.bin si absent du répertoire --vindex (FFN + index ailleurs)",
    )
    ...
    vix = Vindex.load(args.vindex, attn_weights=args.attn_weights)
    print(
