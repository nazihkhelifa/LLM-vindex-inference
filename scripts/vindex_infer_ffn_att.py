...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Chemin vers attn_weights.bin si hors du répertoire --vindex",
    )
    ...
    vix = vip.Vindex.load(args.vindex, attn_weights=args.attn_weights)
    print(
    ...
    if args.forward == "full" and vix.attn is None:
        print(
            "Avertissement: mode full mais aucun mmap attention — seul le FFN s'exécute "
            "(placez attn_weights.bin sous le vindex ou passez --attn-weights).",
            file=sys.stderr,
        )
    print()
