...
use crate::kernels::{apply_rope_hf, gelu_tanh, matvec_par, rms_norm_1, rms_norm_qk};
...
fn infer_impl(vindex: &Vindex, token_ids: &[u32], top_k: usize, skip_attention: bool) -> InferenceResult {
    ...
    for layer in 0..nl {
                ...
                for tok in 0..nt {
                    ...
                    for hi in 0..nq {
                        ...
                        let scores: Vec<f32> = (0..=tok)
                            .map(|j| {
                                (0..hd)
                                    .map(|d| all_q[tok][qs + d] * all_k[j][ks + d])
                                    .sum::<f32>()
                                    * attn_scale
                            })
                            .collect();
        ...
        for tok in 0..nt {
            ...
            for (r, d) in residuals[tok].iter_mut().zip(nf.iter()) {
                *r += d;
            }
        }

        if (layer + 1) % 10 == 0 || layer == nl - 1 {
            let norm = residuals[nt - 1]
                .iter()
